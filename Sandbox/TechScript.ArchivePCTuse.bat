...
echo Date, Name, Start Time, End Time, Total  >> %copyToDir%\%masterLogName%
type %copyToDir%\IOS01.%copyDateStamp%.%appliance%.csv >> %copyToDir%\%masterLogName%
type %copyToDir%\IOS02.%copyDateStamp%.%appliance%.csv >> %copyToDir%\%masterLogName%
type %copyToDir%\IOS03.%copyDateStamp%.%appliance%.csv >> %copyToDir%\%masterLogName%
type %copyToDir%\SDAAR.%copyDateStamp%.%appliance%.csv >> %copyToDir%\%masterLogName%

REM COPY MASTERLOG CONTENT TO DATAMINEUSELOG
::TODO - will save logs to an eternal log book for use-statistics
